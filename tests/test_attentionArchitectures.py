...
def test_tokenShapeAndWidth():
    bits = makeBits(6, 6)
    k = torch.full((2,), 8.0)
    tokens = buildTokenFeatures(bits, 6, 6, k)
...
def test_bitAndGroupColumns():
    l = m = 6
    bits = makeBits(l, m)
    tokens = buildTokenFeatures(bits, l, m, torch.full((2,), 8.0))
...
def test_cyclicPositionIsFractionOfPeriod():
    ...
    kSix = torch.full((1,), 8.0)
    tokensSix = buildTokenFeatures(torch.zeros(1, 24), 6, 6, kSix)
    tokensTwelve = buildTokenFeatures(torch.zeros(1, 48), 12, 12, kSix)
    torch.testing.assert_close(tokensSix[0, 3, POSITIONAL_SLICE],
                               tokensTwelve[0, 6, POSITIONAL_SLICE])


def test_globalsColumns():
    tokens = buildTokenFeatures(torch.zeros(1, 24), 6, 6, torch.full((1,), 72.0))
    torch.testing.assert_close(tokens[0, 0, 12], torch.tensor(math.log(6.0)))
    torch.testing.assert_close(tokens[0, 0, 14], torch.tensor(2.0))  # clip(72/6, 0, 2)
    torch.testing.assert_close(tokens[0, 0, 15], torch.tensor(1.0))
